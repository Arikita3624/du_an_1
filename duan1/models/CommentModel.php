...
class CommentModel
{
    ...
    // Đếm tổng số bình luận đã duyệt cho sản phẩm
    public function countCommentsByProductId($product_id)
    {
        $sql = "SELECT COUNT(*) AS total FROM comments WHERE product_id = :product_id AND status = 'approved'";
        $result = $this->db->query($sql, ['product_id' => $product_id]);
        return $result[0]['total'] ?? 0;
    }

    // Lấy bình luận đã duyệt theo trang
    public function getCommentsByProductIdPaging($product_id, $limit, $offset)
    {
        $sql = "SELECT c.*, u.username
        FROM comments c
        JOIN users u ON c.user_id = u.id
        WHERE c.product_id = :product_id AND c.status = 'approved'
        ORDER BY c.created_at DESC
